# BlueWave Cargo — deploying to Vercel and Neon
...
2. Create the database `bluewave`.
...
| `NEXT_PUBLIC_SITE_URL` | `https://bluewavecargo.co.tz` | the preview domain | required, https, not localhost; built into the bundle, so redeploy after changing it. It is inside every QR code on labels and pickup notes. |
...
ADMIN_EMAIL="owner@bluewavecargo.co.tz" ADMIN_PASSWORD="<12+ chars, mixed>" \
...
Settings → Domains → add `bluewavecargo.co.tz` (and `www` redirecting to it).
