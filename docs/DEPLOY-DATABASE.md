# BlueWave Cargo — the production database
...
1. Create a project in the region closest to the Vercel functions (the app's
   ...
   round trips).
2. Create the database `bluewave` on the default branch (`main`). Leave
...
```
DATABASE_URL="postgresql://USER:PASSWORD@ep-xxxx-pooler.REGION.aws.neon.tech/bluewave?sslmode=require&pgbouncer=true&connection_limit=5&pool_timeout=20"
DIRECT_URL="postgresql://USER:PASSWORD@ep-xxxx.REGION.aws.neon.tech/bluewave?sslmode=require"
```
...
```bash
createdb bluewave_shadow
npx prisma migrate diff --from-migrations prisma/migrations \
  --to-schema-datamodel prisma/schema.prisma \
  --shadow-database-url "postgresql://…/bluewave_shadow" --exit-code   # "No difference detected."
dropdb bluewave_shadow
```
...
- **Warehouses.** Foshan (`GZ`) and Dar es Salaam (`DAR`).
...
- **Off-site copy.** A nightly logical dump kept outside Neon covers losing the
  ...
  ```bash
  pg_dump "$DIRECT_URL" -Fc -f bluewave-$(date +%F).dump    # restore: pg_restore --no-owner -d <url> file.dump
  ```
