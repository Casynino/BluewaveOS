...
export function SupplierAddressCard({
  ...
  return (
    <div className={cn("overflow-hidden rounded-2xl border", dark ? "border-white/20 bg-white text-slate-900" : "bg-card")}>
      <div className="flex items-center justify-between gap-3 border-b bg-[#0a3350] px-4 py-2.5 text-white">
        <p className="text-sm font-bold">【BlueWave Cargo 佛山仓库】</p>
        <button
          type="button"
          onClick={async () => {
            try {
          ...
          className="inline-flex items-center gap-1.5 rounded-full bg-[#ea4a5c] px-3 py-1.5 text-xs font-semibold text-white hover:bg-[#dc4e12]"
        >
