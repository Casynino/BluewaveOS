  ...
  if (waiting.length === 0) {
    return (
      <p className="text-sm text-muted-foreground">
        {tx("Nothing is waiting for a container in Foshan.")}
      </p>
    );
  }
...
const NEXT_STEP: Record<string, Step | null> = {
  ...
  SEALED: {
    to: "DEPARTED",
    heading: "Sealed — ready to leave Foshan",
    ...
    icon: <Ship />,
    waiting: "Foshan records the departure.",
  },
  DEPARTED: {
    to: "ARRIVED",
    ...
    <form action={action} className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <div className="space-y-2">
          ...
          />
          <p className="text-xs text-muted-foreground">
            {tx("The day Foshan stops taking cargo for this sailing. It is published, so moving it goes on the box's timeline.")}
          </p>
        </div>
      </div>
      <div className="space-y-2">
