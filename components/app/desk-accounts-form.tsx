  ...
  return (
    <form action={action} className="space-y-3 rounded-xl border bg-card p-5 shadow-soft">
      <div className="flex items-center gap-2 font-semibold">
        <Users className="size-4" /> Department logins
      </div>
      <p className="text-sm text-muted-foreground">
        ...
        <span className="font-mono text-xs">finance@bluewavecargo.co.tz</span> — all with the password below.
      </p>
      <div className="space-y-2">
        <Label htmlFor="desk-password">Password for all of them</Label>
        <Input id="desk-password" name="password" type="text" minLength={8} required autoComplete="off" />
      </div>
      <FormMessage {...state} />
      <SubmitButton className="w-full">Create department logins</SubmitButton>
    </form>
  );
}
