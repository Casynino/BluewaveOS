 ...
 *
 * Laid out after the Foshan "PACKING LIST & INVOICE" sheet the business has
...
export function PackingListSheet({
  ...
  /*
    ...
    The layout of the paper list Foshan has always sent: the customer's
    ...
    <div className="mx-auto max-w-[860px] space-y-6 print:max-w-none print:space-y-0">
      ...
      */}
      ...
      <article className="pl-sheet mx-auto bg-white p-[10mm] text-[#0b1b2b] shadow-raised ring-1 ring-black/5 print:shadow-none print:ring-0">
        {/* ------------------------------------------------------ Letterhead */}
        <header className="flex items-start justify-between gap-4 border-b-[3px] border-[#0a3350] pb-3">
          <div className="flex items-center gap-3">
            <Image src="/brand/bluewave-cargo.png" alt="" width={58} height={58} className="object-contain" />
            <div>
              <p className="text-[19px] font-extrabold uppercase leading-none tracking-[0.1em] text-[#0a3350]">
                {company?.name ?? "BlueWave Cargo"}
              </p>
              <p className="mt-0.5 text-[8.5px] font-semibold uppercase tracking-[0.2em] text-[#ea4a5c]">
          ...
          <div className="text-right">
            <p className="text-[17px] font-extrabold uppercase leading-none tracking-tight text-[#0a3350]">
              Packing list
            </p>
            <p className="text-[9px] font-bold uppercase tracking-[0.2em] text-[#ea4a5c]">&amp; invoice</p>
        ...
        <section className="mt-3 grid grid-cols-2 gap-2">
          {(
            ...
            <div key={label} className="rounded-md border border-[#d6e2ee] bg-[#f5f9fc] px-3 py-2">
              <p className="text-[7px] font-bold uppercase tracking-[0.2em] text-[#ea4a5c]">{label}</p>
        ...
        {/* ---------------------------------------------- At a glance */}
        <section className="mt-2 grid grid-cols-7 overflow-hidden rounded-md bg-[#0a3350] text-white">
          {[
        ...
        <table className="mt-2.5 w-full border-collapse text-[8.5px]">
          ...
          <thead>
            <tr className="bg-[#0a3350] text-left text-white">
                    ...
                    {ri === 0 ? (
                      <td rowSpan={group.rows.length} className={`${td} border-r bg-white`}>
                        <p className="flex items-start gap-1 font-extrabold uppercase leading-tight">
                          <span className="tnum mt-px grid size-3.5 shrink-0 place-items-center rounded-full bg-[#ea4a5c] text-[6.5px] text-white">
                            {gi + 1}
                          </span>
                          {group.customer}
                        </p>
                        ...
                        {groupMark.trim().toLowerCase() !== group.customer.trim().toLowerCase() ? (
                          <p className="font-mono text-[7.5px] uppercase text-[#0a3350]">Mark: {groupMark}</p>
                        ) : null}
                      </td>
                    ) : null}
          ...
          <tbody>
            <tr className="bg-[#0a3350] font-extrabold text-white">
        ...
        <section className="mt-4 break-inside-avoid">
          <div className="grid grid-cols-3 gap-5">
            {["Prepared by (Foshan)", "Checked by", "Received at Dar es Salaam"].map((label) => (
              <div key={label}>
          ...
          <footer className="mt-4 flex items-end justify-between gap-4 border-t border-[#d6e2ee] pt-2 text-[7px] text-neutral-500">
            ...
            <p className="shrink-0 font-bold uppercase tracking-[0.18em] text-[#0a3350]">
              {company?.name ?? "BlueWave Cargo"}
            </p>
          </footer>
        </section>
