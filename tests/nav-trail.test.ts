...
describe("where Back goes", () => {
  ...
  test("a record is named by the title it showed, a list by its own name", () => {
    rememberTitle("/app/containers/c1", "MSCU 123456-7");
    assert.equal(backLabel("/app/containers/c1"), "MSCU 123456-7");
  ...
  test("a stored value that is not one of our pages is ignored", () => {
    store.set("bw.nav.trail", JSON.stringify(["https://evil.example", "//evil", "/app/customers"]));
