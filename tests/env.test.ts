...
const complete = {
  NODE_ENV: "production",
  DATABASE_URL: "postgresql://u:p@ep-x-pooler.neon.tech/bluewave?sslmode=require",
  DIRECT_URL: "postgresql://u:p@ep-x.neon.tech/bluewave?sslmode=require",
  AUTH_SECRET: "a".repeat(44),
  NEXT_PUBLIC_SITE_URL: "https://bluewavecargo.co.tz",
};
...
describe("production environment", () => {
  ...
  test("the address typed without https:// is accepted; http:// is not", () => {
    /* "www.bluewavecargo.co.tz" was saved in the dashboard exactly like this, and
       every production build failed on it until the scheme was supplied. */
    assert.deepEqual(
      environmentProblems({ ...complete, NEXT_PUBLIC_SITE_URL: "www.bluewavecargo.co.tz" }),
      []
    );
    assert.ok(
      environmentProblems({ ...complete, NEXT_PUBLIC_SITE_URL: "http://www.bluewavecargo.co.tz" }).some(
