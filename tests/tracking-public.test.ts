...
function source(over: Partial<Source> = {}): Source {
  return {
    ...
    containerLines: [
      { container: { reference: "BWC-CNT-0007", shipment: { vessel: "MSC Ilona" } } },
    ],
    history: [
...
const ACCOUNTS = [
  {
    ...
    accountName: "BLUEWAVE CARGO TZ LIMITED",
...
describe("what a reference publishes", () => {
  test("the consignment, the bill and the photographs are on it", () => {
    ...
    assert.equal(result.reference, "BW0001");
    ...
    assert.equal(result.containerReference, "BWC-CNT-0007");
