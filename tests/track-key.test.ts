...
describe("the key in a customer's tracking link", () => {
  test("opens the reference it was made for, in any case", () => {
    const key = trackKey("BW0019");
    assert.ok(trackKeyValid("BW0019", key));
    assert.ok(trackKeyValid("bw0019", key));
  });

  test("opens nothing else: the next reference needs its own key", () => {
    /* References are a counter. A key that worked on BW0020 would let anybody
       holding one bill walk through everybody's. */
    assert.equal(trackKeyValid("BW0020", trackKey("BW0019")), false);
  });

  test("a missing, empty or made-up key opens nothing", () => {
    assert.equal(trackKeyValid("BW0019", null), false);
    assert.equal(trackKeyValid("BW0019", ""), false);
    assert.equal(trackKeyValid("BW0019", "AAAAAAAAAAAAAAAA"), false);
    assert.equal(trackKeyValid("BW0019", "x".repeat(500)), false);
  });
});
