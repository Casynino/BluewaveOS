...
describe("the support desk explains; it does not do", () => {
  ...
  const REFUSED: [Permission, string][] = [
    ["receiving.china", "cannot say what physically arrived in Foshan"],
  ...
  test("every desk may add and edit a customer; only the manager and the owner delete one", () => {
    /* The owner's decision: the desk the customer is standing at is the one
       that hears the new number, so every desk corrects details. Removing a
       customer is not the same act and is not handed out with it. */
    ...
    assert.deepEqual(deleters.sort(), ["ADMIN", "MANAGER"]);
  });
...
describe("custody follows the cargo", () => {
  test("Foshan holds the outbound half, Dar the landed half", () => {
    ...
    /* Dar holds both halves: a clerk with a bale in their hands was looking at
       cargo the system still called Foshan's. */
