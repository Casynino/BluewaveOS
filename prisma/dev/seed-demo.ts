...
async function main() {
  ...
  const support = await prisma.user.findUniqueOrThrow({
    where: { email: "support@bluewavecargo.co.tz" },
  });
  ...
  const container = await prisma.$transaction(async (tx) => {
    const reference = `BWC-CN-${Y}-${pad(await seq(tx, `container:${Y}`), 3)}`;
    ...
    await prisma.$transaction(async (tx) => {
      ...
      const customer = await tx.customer.create({
        data: {
          ...
          city: "Dar es Salaam",
          country: "Tanzania",
          shippingMark: `BW-${slug}-${digits}`,
        },
      });

      const reference = `BW-${Y}-${pad(await seq(tx, `cargo:${Y}`))}`;
      const cargo = await tx.cargo.create({
        data: {
          reference,
          qrToken: `BWQdemo${i}${Date.now()}`,
      ...
      await tx.cargoStatusHistory.createMany({
        data: [
          ...
          { cargoId: cargo.id, from: "REGISTERED", to: "RECEIVED_CHINA", reason: "Received at the Foshan warehouse", actorId: china.id, createdAt: daysAgo(26 - i) },
      ...
      await tx.notification.createMany({
        data: [
          { customerId: customer.id, kind: "cargo.received_china", title: `${reference} received in China`, body: `We have received ${qty} package(s) at our Foshan warehouse.`, href: "/portal", createdAt: daysAgo(26 - i) },
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.pickupRequest.create({
      data: {
        ...
        pickupLocation: "Foshan, Nanhai — Yongfu Road electronics market, shop 3B",
        contactName: "Mr Chen",
    ...
    await prisma.user.create({
      data: {
        ...
        passwordHash: await bcrypt.hash(
          process.env.SEED_ADMIN_PASSWORD ?? "BlueWaveCargo2026!",
          10
        ),
