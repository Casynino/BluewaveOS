...
/**
 * THIRTY CONSIGNMENTS ON THE FOSHAN FLOOR.
 *
...
async function main() {
  ...
  const [warehouse, actor, rates] = await Promise.all([
    ...
    prisma.user.findFirst({ where: { email: "china@bluewavecargo.co.tz" } }),
  ...
  if (!actor) throw new Error("No china@bluewavecargo.co.tz user to receive as.");
  ...
  for (let i = 0; i < 30; i++) {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      const cargo = await tx.cargo.create({
        data: {
          reference,
          qrToken: `BWQ${reference.replace(/\D/g, "")}${i}`,
      ...
      await tx.cargoStatusHistory.createMany({
        data: [
          { cargoId: cargo.id, to: "REGISTERED", actorId: actor.id },
          {
            ...
            from: "REGISTERED",
            to: "RECEIVED_CHINA",
            reason: `Received at Foshan against note ${receiptNo}`,
            actorId: actor.id,
          },
        ],
      });
  ...
  console.log(`\nAdded ${made}. ${onFloor} consignments now waiting in Foshan.`);
}
