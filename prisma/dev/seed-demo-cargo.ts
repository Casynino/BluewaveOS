  ...
  for (let i = 0; i < PEOPLE.length; i++) {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      const customer = await tx.customer.create({
        data: { code, fullName: p.name, businessName: p.biz, phone: p.phone, shippingMark: `BW-${slug}-${digits}`, city: "Dar es Salaam" },
      });

      const reference = `BW-${y}-${pad(await seq(tx, `cargo:${y}`))}`;
      const cargo = await tx.cargo.create({
        data: {
          reference, qrToken: `BWQseed${i}${Date.now()}`,
      ...
      await tx.cargoStatusHistory.createMany({ data: [
        ...
        { cargoId: cargo.id, from: "REGISTERED", to: "RECEIVED_CHINA", reason: "Received at the Foshan warehouse" },
      ]});
      ...
      const gz = await tx.warehouse.findUniqueOrThrow({ where: { code: "FS" } });
