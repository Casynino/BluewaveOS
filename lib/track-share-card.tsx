 ...
 */
export const alt = "BlueWave Cargo — shipping from China to Tanzania";
...
const CHART_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 40 ${CHART_W} 340" width="1200" height="567">
  <defs>
    <linearGradient id="lane" x1="1" y1="0" x2="0" y2="1">
      <stop offset="0%" stop-color="#40c0e8" stop-opacity="0.95"/>
      <stop offset="55%" stop-color="#40c0e8" stop-opacity="0.6"/>
      <stop offset="100%" stop-color="#ea4a5c" stop-opacity="0.95"/>
    </linearGradient>
    <radialGradient id="dar">
      <stop offset="0%" stop-color="#ea4a5c" stop-opacity="0.38"/>
      <stop offset="100%" stop-color="#ea4a5c" stop-opacity="0"/>
    </radialGradient>
  </defs>
  <path d="${CHART_LAND_PATH}" fill="#40c0e8" fill-opacity="0.3"/>
  ...
  <g transform="translate(${SHIP.x.toFixed(1)} ${SHIP.y.toFixed(1)}) rotate(${SHIP.angle.toFixed(1)}) scale(1.5)">
    ...
    <path d="${DECK_PATH}" fill="#40c0e8"/>
    <path d="${BRIDGE_PATH}" fill="#ea4a5c"/>
  </g>
  <circle cx="${GZ[0]}" cy="${GZ[1]}" r="4" fill="#40c0e8"/>
  <circle cx="${DAR[0]}" cy="${DAR[1]}" r="5.5" fill="#ea4a5c"/>
  <circle cx="${DAR[0]}" cy="${DAR[1]}" r="11" fill="none" stroke="#ea4a5c" stroke-opacity="0.55" stroke-width="1.4"/>
</svg>`;
    ...
    (
      ...
      >
        ...
        >
          ...
          <div
            style={{ display: "flex", width: 120, height: 3, marginTop: 26, background: "#ea4a5c" }}
          />
