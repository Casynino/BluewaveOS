...
async function chinaWarehouse(): Promise<ActionCard[]> {
  /*
    THE FOUR QUESTIONS A FOSHAN CLERK ASKS AT EIGHT IN THE MORNING.
...
export async function attentionItems(role?: Role): Promise<AttentionRow[]> {
  ...
  */
  ...
  const rows: AttentionRow[] = [
    ...
    },
    {
      ...
      detail:
        "Booked in and still in Foshan. Every day here is a day the customer is counting.",
      href: "/app/inventory?state=waiting",
      tone: "neutral",
    ...
    },
    {
      ...
      detail:
        "Booked in with fewer packages than Foshan recorded. Held from release until somebody explains the gap.",
      href: "/app/receive/dar",
      tone: "bad",
    },
    {
  ...
  */
  ...
  /* The Foshan desk sees Foshan's own failures, as on the air side: a
 ...
 * company-wide figures are somebody else's dashboard — a clerk in Foshan
 * cannot act on money owed in Dar, and a number nobody can act on is furniture.
 */
export async function chinaFloor() {
  const now = new Date();
  /* "Today" is the Foshan floor's day, not the server's: a clerk opening
     the page at 7am Foshan time is still in yesterday by UTC. China keeps
...
async function deskRows(role: Role): Promise<AttentionRow[]> {
  ...
  /* Foshan's queue is the Waiting and Loading rows above, on Target's
