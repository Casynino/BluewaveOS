...
export async function accrueStorage(
  ...
  const cargo = await prisma.cargo.findMany({
    where: {
      ...
      deletedAt: null,
      /*
        THE NIGHTLY RUN LEAVES GOODS THAT HAVE GONE — asked about by name, it
        does not.

        Their clock stopped at the handover, so the figure cannot grow and a
        line added by the schedule would re-open a settled bill for cartons
        that are already out of the door. But a consignment released ON CREDIT
        has gone and still owes: the days it stood on the floor are owed
        whether or not anybody pressed before it left, and the frozen figure is
        the same one every screen shows. Cargo that never arrived is never
        charged either way.
      */
      status: options.cargoIds
        ? { notIn: ["CANCELLED", "MISSING_AT_DAR"] }
        : { notIn: [...GONE] },
      ...(options.cargoIds ? {} : { release: { is: null } }),
      OR: [
        { darArrivedAt: { lte: before } },
        { darArrivedAt: null, darReceiving: { is: { receivedAt: { lte: before } } } },
      ],
    },
    select: {
      id: true,
      ...
      /* The day the goods went, which is the day the clock stopped. */
  ...
  for (const row of cargo) {
    const start = storageStart(row.darReceiving?.receivedAt ?? null, row.darArrivedAt);
    if (!start) continue;
    const position = storagePosition({
      receivedAt: start,
      /* A consignment that has gone stopped costing floor space the day it
         went. Charging to today would bill a customer for a fortnight their
         cartons spent in their own shop. */
      collectedAt: row.release?.releasedAt ?? null,
      freeDays,
      perDay,
      currency,
    });
