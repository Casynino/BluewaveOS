 ...
 *
 * "www.bluewavecargo.co.tz" with no scheme is how an address is usually written,
...
export function labelSiteUrl(): string {
  ...
  if (process.env.NODE_ENV !== "production") return "http://localhost:3188";
  throw new Error(
    "NEXT_PUBLIC_SITE_URL is not set to the public address (for example https://www.bluewavecargo.co.tz). " +
...
export function metadataBaseUrl(): URL | null {
  ...
  return process.env.NODE_ENV === "production" ? null : new URL("http://localhost:3188");
}
