...
export async function scanBoxAtDar(
  ...
  if (!scanned) {
    ...
    return { error: "That code is not a BlueWave Cargo box. Check the label, or type the tracking number to search.", at };
  }
  if (!scanned.box) {
  ...
  if (box.voidedAt) {
    await log("refused", "voided", "This box was taken off its line in Foshan.");
    return { error: `${box.cargo.reference} box ${box.sequence} was taken off its line in Foshan — this label should not be on a box.`, last, at };
  }
  if (box.collectedAt) {
...
export async function scanBoxForRelease(
  ...
  if (!scanned?.box) {
    ...
    return { error: scanned ? "That is not a box label. Scan the sticker on the box." : "That code is not a BlueWave Cargo box.", at };
  }
