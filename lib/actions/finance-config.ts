...
const SETTING_FIELDS = {
  ...
  instagram: "Instagram",
  email: "Email",
  chinaAddress: "Foshan warehouse address",
...
const settingsSchema = z.object({
  ...
  whatsapp: z
    .string()
    .trim()
    .regex(/^\d{9,15}$/, "WhatsApp is digits only, with the country code — 255688887784.")
    .or(z.literal(""))
    .optional(),
  /* Typed however it was copied — "@bluewave_cargo" or the whole profile
     address — and kept as the bare handle the link is built from. */
  instagram: z
    .string()
    .trim()
    .transform((v) => v.replace(/^https?:\/\/(www\.)?instagram\.com\//i, "").replace(/^@/, "").replace(/[/?].*$/, ""))
    .pipe(z.string().regex(/^[A-Za-z0-9._]{1,30}$/, "An Instagram handle is letters, digits, dots and underscores.").or(z.literal("")))
    .optional(),
