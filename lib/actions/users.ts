...
export async function createUser(
  ...
  /*
    The warehouse is a Foshan or a Dar floor, and the role already says which.
  ...
  const user = await prisma.$transaction(async (tx) => {
    const created = await tx.user.create({
      data: {
        ...
        /* Foshan opens in Chinese; everyone else in English. A starting
...
export async function changeUserRole(
  ...
  const outcome = await prisma.$transaction(async (tx): Promise<Guarded> => {
    ...
    */
    ...
    // China clerk moved to Finance is no longer narrowed to Foshan.
