 ...
 * connects a box in Foshan to an owner, so two customers must never be able
  ...
  await prisma.customer.update({
    ...
    data: {
      ...
      /* The shipping mark is deliberately NOT recomputed from a changed name.
         Boxes already in Foshan carry the old mark in marker pen, and a mark
...
export async function registerCustomerAtCounter(input: {
  ...
  const customer = await prisma.$transaction(async (tx) => {
    ...
    /*
      ...
      It is what is written on the boxes in Foshan, and what is written on
