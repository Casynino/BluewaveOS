 ...
 *
 * While Foshan holds the cargo, China's receiving CBM is the sum of the
...
export async function syncLineTotals(
  ...
  /* The count and the weight follow the lines as the volume does. A carton
     added to a line while Foshan holds it left China's receiving at the old
 ...
 * holds the cargo: Foshan until Dar checks it in, Dar from then on, and
 * management always. Dar re-measuring a bale corrects the line the same way
 * Foshan would, and the bill follows the corrected line.
 *
...
export async function applyPackageLine(
  ...
  /* A line that did not exist has no old value, but the new one is still a
     ...
     carton Foshan never wrote down. */
  await recordFieldChange(
    {
      actor,
      entity: "CargoPackage",
 ...
 *
 * Custody as everywhere else: Foshan corrects cargo still on its side, Dar
