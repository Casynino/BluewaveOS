...
/**
 * THE TRACKING NUMBER: BW0001, BW0002, BW0003.
 *
 * Six characters, said down a phone line in one breath. BW-2026-000125 was
 ...
 * and never resets — BW0999 in December is followed by BW1000 in January, and
 * a number is never reissued to a second consignment.
 */
export async function nextCargoReference(tx: TxClient) {
  return `BW${pad(await nextSequence(tx, "cargo"), 4)}`;
}

/** A line within it. BW0125-P3. Composed, not counted. */
...
/**
 * The customer's shipping mark: BW-NINO-125.
 *
 * Built from a name, not counted, because the point of a mark is that a factory
 * worker in Foshan can write it on a box with a marker pen and a clerk can
...
/**
 * OUR OWN NAME FOR THE BOX: BWC26M09C5.
 *
...
export async function nextContainerReference(tx: TxClient, at = new Date()) {
  ...
  return `BWC${yy}M${mm}C${await nextSequence(tx, "container")}`;
}
...
/** A business visit asked for off the website: BV-2026-000001. */
export async function nextVisitReference(tx: TxClient, y = year()) {
  return `BV-${y}-${pad(await nextSequence(tx, `visit:${y}`))}`;
}

/**
 * The secret half of a status link sent to somebody who asked for a visit or
 * sourcing off the website. References run in sequence; this does not, so
 * holding one request's link opens that request and nothing else.
 */
...
export function generateQrToken() {
  return `BWQ${randomBytes(20).toString("base64url")}`;
}
