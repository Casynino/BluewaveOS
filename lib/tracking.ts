 ...
 * type BW0001, BW0002, BW0003 in an afternoon and read one customer's amount
 ...
 * belongs, and sometimes as the package label (BW0125-P3) rather than the
 * consignment.
 */
export function referenceFromInput(raw: string): string | null {
  ...
  const onLabel = code.match(/^(BW-?[0-9O]+)-P\d+$/);
  const base = onLabel ? onLabel[1] : code;

  /* BW62 is BW0062 said aloud: the leading zeros are print padding, and the
     counter never reissues a number, so the two cannot name different cargo. */
  const sc = base.match(/^BW-?([0-9O]+)$/);
  if (sc) return `BW${sc[1].replace(/O/g, "0").padStart(4, "0")}`;

  /* Older printed references (BW-2026-000125) are still valid names for a
...
function noteFor(input: {
  ...
  return {
    sw: "Tunausubiri mzigo wako Foshan.",
