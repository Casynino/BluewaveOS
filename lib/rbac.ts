...
export type Permission =
   ...
   *
   * `amendChina` covers cargo still on the Foshan shelf or on the water.
   ...
   *
   * Foshan holds the outbound half because it typed the record and is who
   ...
   * the system still called Foshan's. Foshan holds only its own half.
   */
  | "cargo.amendChina"
  | "cargo.amendDar"
   ...
   *
   * Loading is Foshan's and stops at the seal — `container.load` refuses
...
/**
 * Foshan. Receive, identify, record, photograph, load, seal, dispatch.
 *
 * It does not scan: labels are read in Dar, against a manifest. In Foshan
...
const CUSTOMER_SUPPORT: Permission[] = [
  ...
  /*
    ...
    can see boxes in Foshan, so every figure entered here would be somebody
