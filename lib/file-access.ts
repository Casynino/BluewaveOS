...
export async function fileAccess(
  ...
  /* Pictures on the public China guide: a published city, market gallery,
     factory or category shows them to anybody. */
  const [cityImages, marketGallery, factoryImages, categoryImages] = await Promise.all([
    prisma.chinaCity.count({ where: { published: true, OR: [{ heroImage: url }, { gallery: { has: url } }] } }),
    prisma.marketInformation.count({ where: { published: true, gallery: { has: url } } }),
    prisma.factory.count({ where: { published: true, OR: [{ heroImage: url }, { gallery: { has: url } }] } }),
    prisma.productCategory.count({ where: { published: true, image: url } }),
  ]);
  if (cityImages + marketGallery + factoryImages + categoryImages > 0) return "public";
  ...
  */
  ...
  /* A proforma sent in against an enquiry, or the photograph the Foshan
