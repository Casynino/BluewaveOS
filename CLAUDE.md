...
Sea freight ERP: Foshan → Dar es Salaam. Loose cargo (LCL) and full
...
- A proven production sea-freight system rebuilt for BlueWave: the business
  logic is carried over unchanged, the identity is BlueWave's. See
  `docs/ARCHITECTURE.md` §1 for what is BlueWave's own and what is still to be
  confirmed by the company.
- Company facts (name, phones, addresses, TIN, accounts, VAT) start in
  `prisma/data/company.ts` and live in `CompanySetting` afterwards. Never
  hard-code them in a component.
...
Foshan corrects a consignment until Dar books it in; Dar corrects it from
then on, and — by the owner's decision — may also correct one not yet booked
in. Dar edits the package lines just as Foshan does: type, count, sides,
...
`lib/i18n.ts` carries the Chinese for the Foshan floor, which opens in Chinese
(`defaultLocaleForRole`) and can switch at any time. Status keys, references,
numbers and money are never translated. Cargo descriptions keep their original
and show a translation beside it (`lib/locale.ts` `pickText`), so the words a
customer disputes are never lost. `npx tsx scripts/i18n-missing.ts` lists
strings still without Chinese.
...
Local Postgres (`bluewave`, demo data in `bluewave_test` — see docs/TESTING.md), dev server on `npx next dev -p 3188`. Drive the
real forms in a browser rather than trusting that it compiles. Seeded staff are
`<desk>@bluewavecargo.co.tz` with `SEED_ADMIN_PASSWORD` from `.env`:
