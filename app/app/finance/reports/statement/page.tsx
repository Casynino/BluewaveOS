...
export async function generateMetadata({
  ...
  return { title: { absolute: `BlueWave Cargo - Financial statement - $<Tx>{r.label}</Tx>` } };
}
      ...
      <article className="rounded-lg border bg-white p-10 text-black print:border-0 print:p-0">
        <header className="flex items-start justify-between gap-6 border-b-2 border-black pb-5">
          <div className="flex items-center gap-3">
            <Image src="/brand/bluewave-cargo.png" alt="" width={56} height={56} className="object-contain" />
            <div>
              <p className="text-xl font-bold uppercase tracking-wider">{T("BlueWave Cargo")}</p>
              <p className="text-xs text-neutral-600">{T("Foshan → Dar es Salaam")}</p>
            </div>
          </div>
        ...
        <Section title="7. Collections and volume">
          ...
          <Line l="CBM received in Foshan" m={`${f.cbmReceived.toFixed(3)} CBM`} />
