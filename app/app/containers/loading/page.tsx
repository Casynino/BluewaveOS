...
/**
 * THREE STAGES OF A BOX IN FOSHAN.
 *
 ...
 *
 * Every container that is still in Foshan or has just sailed, with what is
 * inside each one a click away on this page — the loading bay should not have
 * to open six tabs to answer "is BW0041 on the box or still on the floor".
 *
...
export default async function LoadingContainersPage({
  ...
  const [containers, unassigned] = await Promise.all([
    ...
    /* Measured in Foshan and on no container. Receiving never asks which
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description={T("Every box still in Foshan and the ones that have just sailed. Open one to see exactly what is inside.")}
        actions={
      ...
      {boxes.length === 0 ? (
        <div className="rounded-xl border bg-card">
          <EmptyState
            ...
            description={T("Cargo measured in Foshan waits on the floor until a container is opened for it.")}
          />
        </div>
      ) : null}
                  ...
                  <div className="border-t">
                    {box.lines.length === 0 ? (
                      <p className="px-5 py-6 text-sm text-muted-foreground">
                        {T("Nothing loaded yet. Pick cargo off the Foshan floor from inside the container.")}
                      </p>
                    ) : (
